...
``` yaml { .animate src="./files/images/06__research_design.svg" }
setup:
...
\mathbf{y}
= 
\underbrace{\mathbf{X \; \beta}}_{\text{Fixed}} 
\; + \;
\underbrace{\mathbf{Z \; u}}_{\text{Random}}
\;+\;
\mathbf{\varepsilon}, \qquad
...
::: {.notes}
- When analysing psychological data, we often have **multiple observations nested within individuals** — for example, repeated ratings or responses. 
- A standard regression model assumes that every observation comes from the same population and can therefore be described by one common intercept and slope.
- It works well when observations are independent, but it ignores the fact that responses from the same person are usually more similar to each other than to those from different people.
:::
...
::: {notes}
- A multilevel model **relaxes this assumption** by allowing certain parameters — typically the intercepts or slopes — to vary across groups, such as participants.
:::
